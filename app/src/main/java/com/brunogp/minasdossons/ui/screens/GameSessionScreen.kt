...
fun GameSessionScreen(
    ...
    val questions =
        remember(world, level, progress.easyMode, allPairs) {
    ...
    fun answer(value: String) {
        if (value == q.correctAnswer) {
            if (!tried) stars++
            feedback =
                if (q.type == QuestionType.THROAT_VIBRATES) q.explanation else listOf("Boa!", "Muito bem!", "Grande trabalho!").random()
        ...
        when (q.type) {
            QuestionType.RECORD_SOUND -> {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    BlockButton("Tentar outra vez", {
                        tried = true
                        feedback = "Vamos ouvir outra vez"
                    }, Modifier.weight(1f), color = Color(0xFF607D8B))
                }
            }
