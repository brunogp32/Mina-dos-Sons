        ...
        worlds.forEachIndexed { index, world ->
            ...
            PixelCard(color = if (unlocked) Color(0xFFFFF7D7) else Color(0xFFD0C8B6)) {
                ...
                if (number ==
                    ...
                    Text(
                        (LocalGameData.minimalPairs + progress.customMinimalPairs).joinToString("  •  ") { "${it.wordA}/${it.wordB}" },
                        fontSize = 14.sp,
                    )
                }
                for (levels in (1..10).chunked(5)) {
                    Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                        for (level in levels) {
                            BlockButton(if (unlocked) "N$level" else "🔒", {
                                nav.navigate("game/$number/$level")
                            }, Modifier.weight(1f), enabled = unlocked, color = if (unlocked) Color(0xFF5AA469) else Color.Gray)
                        }
                    }
                }
            }
