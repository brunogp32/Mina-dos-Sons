...
object CardCatalog {
    ...
    fun normalizeOwnedIds(ids: Set<String>): Set<String> =
        ids.map { old -> card(old)?.id ?: old }.filter { card(it) != null }.toSet()

    private fun buildCatalog(): List<CardItem> {
        val specs = listOf(
            FocusSpec(SoundFocus.S, "Reino da Cobra", "S", listOf("sapo", "saco", "selo", "sino", "sopa", "suma", "sala", "seda", "sombra", "serra")),
            FocusSpec(SoundFocus.Z, "Colmeia do Z", "Z", listOf("zebra", "zero", "azeite", "azul", "rosa", "casa", "mesa", "brasa", "tesoura", "vizinho")),
            FocusSpec(SoundFocus.X_CH, "Nuvens do X", "X", listOf("chá", "chave", "chuva", "peixe", "caixa", "baixo", "xarope", "mexer", "lixo", "fecho")),
            FocusSpec(SoundFocus.J, "Jardim do J", "J", listOf("janela", "jogo", "jardim", "joaninha", "jornal", "girafa", "gelado", "gelo", "viagem", "loja")),
            FocusSpec(SoundFocus.MIXED, "Mestres da Fala", "M", listOf("voz", "garganta", "escuta", "sílabas", "palavras", "frases", "eco", "som", "mina", "tesouro")),
        )
    ...
    private data class FocusSpec(val focus: SoundFocus, val collection: String, val prefix: String, val words: List<String>)

    private val palette = listOf(
        0xFF2F7D32, 0xFFD6A22A, 0xFF4A90A4, 0xFFD24D57, 0xFF8B6BB1,
        0xFF795548, 0xFF607D8B, 0xFF43A047, 0xFFFF8A65, 0xFF26A69A,
    )
    ...
    private fun priceFor(rarity: CardRarity, index: Int): Int {
    ...
    private fun illustrationFor(focus: SoundFocus, index: Int): CardIllustrationType = when (focus) {
        SoundFocus.S -> if (index % 3 == 0) CardIllustrationType.SNAKE_SOUND else CardIllustrationType.LETTER
        SoundFocus.Z -> if (index % 3 == 0) CardIllustrationType.BEE_SOUND else CardIllustrationType.WAVE
        SoundFocus.X_CH -> if (index % 3 == 0) CardIllustrationType.RAIN_SOUND else CardIllustrationType.WORD
        SoundFocus.J -> if (index % 3 == 0) CardIllustrationType.LADYBIRD_SOUND else CardIllustrationType.SYLLABLE
        SoundFocus.MIXED -> listOf(CardIllustrationType.CRYSTAL, CardIllustrationType.MINE_TREASURE, CardIllustrationType.VOICE, CardIllustrationType.LISTENING)[index % 4]
    }

    private fun nameFor(spec: FocusSpec, word: String, index: Int): String {
        val themes = when (spec.focus) {
            SoundFocus.S -> listOf("Sopro da Cobra", "Eco do S", "Sapo Saltador", "Saco Surpresa", "Serpente Suave")
            SoundFocus.Z -> listOf("Zumbido Dourado", "Eco do Z", "Zebra Azul", "Rosa Sonora", "Ziguezague da Voz")
            SoundFocus.X_CH -> listOf("Chuva Suave", "Chave de Cristal", "Peixe do X", "Caixa Sonora", "Nuvem do CH")
            SoundFocus.J -> listOf("Janela Brilhante", "Jardim das Palavras", "Joaninha do J", "Jogo da Voz", "Jóia Sonora")
            SoundFocus.MIXED -> listOf("Mestre da Vibração", "Guardião das Sílabas", "Tesouro da Voz", "Ouvinte Especial", "Cristal dos Sons")
        }
        return if (index < themes.size) themes[index] else "${themes[index % themes.size]} ${word.replaceFirstChar { it.uppercase() }}"
    }

    private fun descriptionFor(focus: SoundFocus, word: String, collection: String): String =
        "Carta da coleção $collection para treinar ${focus.label} com a palavra $word."
